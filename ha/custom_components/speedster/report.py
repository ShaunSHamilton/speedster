...
def _template_path() -> Path:
    ...
    for candidate in (
        here / TEMPLATE_NAME,
        here.parents[2] / TEMPLATE_NAME,  # repo root, when running from a checkout
    ):
...
def build(csv_path: Path, config_dir: Path, options: dict[str, object]) -> Path:
    ...
    meta = (
        "{interval:"
        + str(int(options[CONF_INTERVAL_MINUTES]))  # type: ignore[arg-type]
        + ",paused:"
